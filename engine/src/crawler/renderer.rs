...
// The SPA render fallback below spawns a Node.js worker and talks to it
// over a Unix domain socket -- tokio only implements `UnixStream` for Unix
// targets (it doesn't exist in `tokio::net` at all on Windows), so this
// whole IPC mechanism only builds on Unix. `render_via_node`'s caller in
// engine/src/lib.rs already treats any error from it as a soft failure and
// continues with fast-path-only (unrendered) data, so the `#[cfg(not(unix))]`
// stub below -- which always returns such an error -- reuses that same
// existing degradation path rather than needing one of its own.
#[cfg(unix)]
mod unix_worker {
...
#[cfg(unix)]
pub use unix_worker::render_via_node;

#[cfg(not(unix))]
pub async fn render_via_node(url: &str, _node_cli_path: &str, _timeout_ms: u64) -> anyhow::Result<String> {
    Err(anyhow::anyhow!(
        "SPA render fallback isn't supported on this platform yet (needs a non-Unix-socket worker IPC path); skipping for {url}"
    ))
}
